 ...
 * FINAL PRODUCTION VERSION: Integrated with students.json, Class Modal, new Reg No format, and Watermark update.
 */
...
const app = {
    ...
    populateRegNumberDropdown: (className) => {
        ...
        if (filteredStudents.length > 0) {
            ...
            const displayClass = className.replace(/(\w{3})(\d)/, '$1 $2');
            option.textContent = `No students found for ${displayClass}. Select for Manual Entry.`;
            regSelect.appendChild(option);
            regSelect.disabled = false;
...
const pdfGenerator = {
    ...
    updateMetrics: () => {
        // Recalculates metrics for PDF generation
    },
    ...
    createPDF: async () => {
        ...
        doc.setFontSize(8).setTextColor(150, 150, 150).setFont("helvetica", "bold");
        doc.text("Any alteration renders it invalid", 105, footerY, {align: 'center'});

        // 6.2. Powered By (Normal)
        footerY += 3;
        doc.setFontSize(8).setTextColor(150, 150, 150).setFont("helvetica", "normal");
        doc.text("Powered by PraixTech", 105, footerY, {align: 'center'});
